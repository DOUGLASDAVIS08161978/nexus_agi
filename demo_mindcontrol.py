...
def main():
    ...
    for i, agent in enumerate(agents):
        ...
        print(f"   └─ {agent}: {result['status']} (effectiveness: {result['estimated_effectiveness']:.1%})")
    print()
    ...
    for agent, enhancement_type in enhancements:
        ...
        print(f"   └─ {agent}: {enhancement_type} enhanced to level {result['enhanced_level']:.2f}")
    print()
    ...
    if sync_result['emergent_properties']:
        print(f"   └─ Emergent Properties: {', '.join(sync_result['emergent_properties'])}")
    print()
