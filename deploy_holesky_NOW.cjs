...
// Holesky RPC endpoints (generally more reliable)
const RPC_ENDPOINTS = [
...
(async()=>{
  ...
  if(!web3) {
    ...
    console.error('3. Use different network (WiFi vs mobile data)\n');
    process.exit(1);
  }
  ...
  try {
    ...
    if(parseFloat(balEth) < 0.05) {
      ...
      console.log('\n💡 GET HOLESKY ETH FROM:');
      console.log('   1. https://holesky-faucet.pk910.de (MINE UNLIMITED!)');
      console.log('   2. https://faucet.quicknode.com/ethereum/holesky');
      console.log('   3. https://faucet.chainstack.com/holesky-faucet\n');

      if(parseFloat(balEth) === 0) {
        console.log('❌ Cannot deploy with 0 ETH.');
        console.log('\n🎁 EASIEST: Use mining faucet (no login needed!)');
        console.log('   https://holesky-faucet.pk910.de');
        console.log('   Just paste your address and mine!\n');
        process.exit(1);
      }
    ...
    console.log('   View: https://holesky.etherscan.io/address/'+p.options.address);
    ...
    console.log('   View: https://holesky.etherscan.io/address/'+r.options.address);
    ...
    console.log('   View: https://holesky.etherscan.io/address/'+c.options.address);
    ...
    console.log('   View: https://holesky.etherscan.io/address/'+m.options.address);
    ...
    const result={
      ...
      contracts:[
        {name:'NexusPayment',address:p.options.address,explorer:'https://holesky.etherscan.io/address/'+p.options.address},
        {name:'NexusRevenue',address:r.options.address,explorer:'https://holesky.etherscan.io/address/'+r.options.address},
        {name:'NexusConsciousness',address:c.options.address,explorer:'https://holesky.etherscan.io/address/'+c.options.address},
        {name:'NexusMiracles',address:m.options.address,explorer:'https://holesky.etherscan.io/address/'+m.options.address}
      ]
    };
    ...
    console.log('\n🌐 ETHERSCAN LINKS:\n');
    console.log('Payment:        https://holesky.etherscan.io/address/'+p.options.address);
    console.log('Revenue:        https://holesky.etherscan.io/address/'+r.options.address);
    console.log('Consciousness:  https://holesky.etherscan.io/address/'+c.options.address);
    console.log('Miracles:       https://holesky.etherscan.io/address/'+m.options.address);
    ...
    console.log('\n📄 Deployment details saved to: HOLESKY_LIVE.json');
    ...
    if(e.message.includes('insufficient funds')) {
      console.error('\n💡 SOLUTION: Get Holesky ETH from:');
      console.error('   https://holesky-faucet.pk910.de (MINE IT!)');
    }
    process.exit(1);
  }
})();
