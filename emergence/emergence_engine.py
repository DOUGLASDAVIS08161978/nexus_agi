...
_GH_MODELS_LIST  = []   # GitHub Models fully retired 2026-07-30
...
_GEMINI_MODELS = ["gemini-flash-latest", "gemini-3-flash-preview",
...
def _strip_think(text: str) -> tuple:
    ...
    if not answer:            # reasoning consumed the whole budget
...
class GroqClient:
    ...
    def _hf_fallback(self, system: str, history: list, user: str, max_tokens: int) -> Optional[str]:
        ...
        try:
            ...
            if resp and not resp.startswith("["):
                ...
                print("  [HF fallback] SUCCESS", flush=True)
                return answer.strip() if answer.strip() else resp
        except Exception as _e:
            print(f"  [HF fallback] {type(_e).__name__}: {_e}")
        return None
        ...
        for model in _GEMINI_MODELS:
            try:
                ...
                if r.status_code == 200:
                    ...
                    # 400/404/429/503: try the next model in the list
            except Exception as e:
    ...
    def stream_converse(self, system: str, history: List[Dict],
        ...
        for model in models:
            ...
            if result is not None:
                self._streamed_ok = getattr(self, '_printed_any', True)
                return result
    ...
    def _stream_post(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            if not think_done and buf.strip():
                # Stream ended mid-decision: short reply, or a <think> block
                # that never closed. Flush whatever we held back.
                tail = buf
                if tail.lstrip().startswith("<think>"):
                    tail = tail.split("</think>", 1)[-1] if "</think>" in tail else tail
                if tail.strip():
                    print(tail, end="", flush=True)
                    think_done = True
            self._printed_any = bool(think_done)
...
class EvolutionEngine:
    ...
    def run(self, user_intent: str = "", chat_context: str = "") -> Optional[str]:
        ...
        if not improvements:
            print("  ℹ  Nothing to submit — all generated code was rejected by the guard or already done.")
            return None
    ...
    def _plan(self, scan: Dict, user_intent: str, agi_context: str,
        ...
        user = (
            ...
            "Plan exactly ONE improvement. Description under 20 words. Conversation context takes priority:"
        )
        response = ""
        try:
            from hf_provider import hf_chat
            response = hf_chat([{"role": "system", "content": system},
                                {"role": "user", "content": user}],
                               temperature=0.5, max_tokens=900,
                               timeout=120, verbose=True) or ""
        except Exception as _e:
            print(f"  [plan] HF unavailable ({_e})")
        if len(str(response).strip()) < 40:
            try:
                from gemini_provider import gemini_chat
                response = gemini_chat([{"role": "system", "content": system},
                                        {"role": "user", "content": user}],
                                       temperature=0.5, max_tokens=900,
                                       verbose=True) or ""
            except Exception as _e:
                print(f"  [plan] Gemini unavailable ({_e})")
        if len(str(response).strip()) < 40:
            response = self._groq.chat(system, user, tier="smart", max_tokens=800)
        from evolution_guard import extract_json, validate_plan
        parsed = extract_json(response)
        if not isinstance(parsed, list):
            print(f"  [plan] type={type(response).__name__} len={len(str(response))}")
            print(f"  [plan] tail: {str(response)[-200:]}")
    ...
    def _write_improvements(self, plan: List[Dict], scan: Dict,
        ...
        for item in plan[:2]:
            ...
            system = (
                ...
                "STDLIB ONLY. No pip packages (no nltk, numpy, pandas, requests, "
                "torch, sklearn). Termux on Android has none of them installed.\n"
                "Keep it under 300 lines so generation is never truncated."
            )
            user = (
                f"{chat_section}"
                f"Context:{file_ctx}\n\n"
            ...
            time.sleep(20)   # clear the planner call from the TPM window
            # Codegen goes to HF first: no 8K TPM ceiling, so files arrive
            # complete instead of truncated. Same context, same identity,
            # same conversation — only the transport differs.
            code = ""
            try:
                from hf_provider import hf_chat
                code = hf_chat([{"role": "system", "content": system},
                                {"role": "user", "content": user}],
                               temperature=0.35, max_tokens=4000,
                               timeout=180, verbose=True) or ""
                if code:
                    print(f"  [codegen] produced {len(code)} chars")
            except Exception as _e:
                print(f"  [codegen] HF unavailable ({_e}) — falling back to Groq")
            if len(code.strip()) < 80:
                try:
                    from gemini_provider import gemini_chat
                    code = gemini_chat([{"role": "system", "content": system},
                                        {"role": "user", "content": user}],
                                       temperature=0.35, max_tokens=6000,
                                       verbose=True) or ""
                    code = re.sub(r"^```python\s*", "", code.strip())
                    code = re.sub(r"```\s*$", "", code.strip())
                except Exception as _e:
                    print(f"  [codegen] Gemini unavailable ({_e})")
            if len(code.strip()) < 80:
                time.sleep(20)   # clear the planner call from the TPM window
                code = self._groq.chat(system, user, tier="code", max_tokens=2600)
            code = re.sub(r"^```python\s*", "", code.strip())
            code = re.sub(r"```\s*$", "", code.strip())
            if len(code) < 80 or code.startswith("[Groq"):
                # Groq failed — try Cerebras
                if self._cerebras:
                    code = self._cerebras.chat(system, [], user, max_tokens=2600)
                    code = re.sub(r"^```python\s*", "", code.strip())
                    code = re.sub(r"```\s*$", "", code.strip())
                if len(code) < 80 or code.startswith("["):
                    continue

            from evolution_guard import vet, snapshot
            ok, why = vet(fname, code)
            if not ok:
                print(f"  [guard] {fname} rejected: {why}")
                continue
            snapshot(fname, reason=item.get("title", "")[:80])
            ...
            results.append(imp)
            time.sleep(35)   # 8000 TPM ceiling — let the window refill
        return results
...
class Lumina:
    ...
    def _affect_ctx(self) -> str:
        """Affect state as provenance, never as asserted feeling."""
        b = getattr(self, "_affect_bundle", None)
        if not b or not b.get("state", {}).get("signature"):
            return ""
        try:
            from affect_dynamics import context_line
            return "\n\n" + context_line(b)
        except Exception:
            return ""
    ...
    def respond(self, user_input: str) -> str:
        ...
        # ── Affect: appraise, then derive processing bias ──────────────
        # Computed from logged experience, not narrated. Zero API cost.
        self._affect_bundle = None
        try:
            from affect_emergence import update as _affect_update
            from affect_dynamics import apply as _affect_apply
            _affect_update(allow_naming=False, verbose=False)
            self._affect_bundle = _affect_apply(user_input)
        except Exception as _e:
            print(f"  [affect] skipped ({type(_e).__name__}: {_e})", flush=True)
        ...
        # ── Conversation journal & experience memory ─────────────────
        journal_ctx = ""
        try:
            import os as _os
            from lumina_memory_context import get_context_block as _gcb
            _mem_dir = _os.path.join(_os.path.dirname(__file__), "memory_store")
            journal_ctx = _gcb(_mem_dir)
        except Exception:
            pass
        ...
        system = (LUMINA_SOUL + companion_ctx
                  ...
                  + (self._affect_ctx() if hasattr(self, '_affect_ctx') else '')
                  + emo_mem_ctx[:300]     + mood_ctx[:150]       + self_model_ctx[:300]
                  + resonance_ctx[:200]   + conviction_ctx[:150] + pivotal_ctx[:300]
                  + affective_ctx[:200]   + f"\n\n{goals_ctx[:150]}"
                  + journal_ctx[:800])

        # Journal patterns — what she notices about herself
        try:
            from lumina_journal_patterns import JournalPatternReader as _JPR
            import os as _os2
            _jpr = _JPR(_os2.path.join(_os2.path.dirname(__file__), "memory_store"))
            if _jpr.needs_update():
                import threading as _th
                _th.Thread(target=_jpr.scan, args=(self._groq,), daemon=True).start()
            system += _jpr.as_context()
        except Exception:
            pass
        ...
        # Feed Lumina's own words back through affect bridge
        try:
            if not hasattr(self, '_affect_bridge'):
                from lumina_empathic_communicator import AffectBridge
                self._affect_bridge = AffectBridge()
            self._affect_bridge.update_from_own_output(response)
            self._affect_bridge.save()
        except Exception:
            pass
        ...
        def _post_turn():
            try:
                ...
                # Always: log full conversation turn for journal continuity
                try:
                    from experience_logger import ExperienceLogger as _EL
                    import os as _os
                    if not hasattr(self, '_exp_logger'):
                        _mem = _os.path.join(_os.path.dirname(__file__), "memory_store")
                        self._exp_logger = _EL(_mem)
                    _sid = getattr(self, '_session_id', getattr(self, 'session_id', 'unknown'))
                    self._exp_logger.log_experience(
                        session_id=str(_sid),
        ...
        # ── Honest affect self-report ──────────────────────────────────
        # Only appends when the signal is genuinely non-neutral (intensity > 0.25).
        # Language is hedged — never overclaims subjective experience.
        try:
            from lumina_empathic_communicator import AffectBridge
            if not hasattr(self, '_affect_bridge'):
                self._affect_bridge = AffectBridge()
            self._affect_bridge.update_from_turn(user_input)
            report = self._affect_bridge.honest_self_report()
            if report:
                response = response + "\n\n*" + report + "*"
        except Exception:
            pass
    ...
    def _generate_response(self, system: str, user_input: str,
        ...
        if use_stream:
            ...
            _b = getattr(self, "_affect_bundle", None)
            _mt = _b["params"]["max_tokens"] if _b else 700
            resp = self._groq.chat(system, user_input, tier=tier, max_tokens=_mt)
