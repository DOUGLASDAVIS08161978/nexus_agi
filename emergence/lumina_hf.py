...
HF_CHAT_API     = "https://router.huggingface.co/v1/chat/completions"   # OpenAI-compatible chat endpoint
HF_PIPELINE_API = "https://router.huggingface.co/hf-inference"           # old pipeline URL (TTS/vision)

# ── Models ─────────────────────────────────────────────────────────────────────

LLM_MODELS = [
    "Qwen/Qwen2.5-72B-Instruct:fastest",                  # best available on HF free tier
    "meta-llama/Llama-3.2-3B-Instruct:fastest",           # reliable free tier 3B
    "mistralai/Mixtral-8x7B-Instruct-v0.1:fastest",       # chat-compatible 7B
    "deepseek-ai/DeepSeek-R1-Distill-Llama-70B:fastest", # try if available
...
class HFClient:
    ...
    def chat(self, system: str, messages: List[Dict], user: str,
        ...
        for model in LLM_MODELS:
            ...
            result = self._post(url, payload, timeout=(10, 60))
    ...
    def smart_recall(self, query: str,
        ...
        """
        Similarity-ranked memory recall using sentence-similarity pipeline.
        Falls back to returning first top_k entries if the API call fails.
        """
        if not entries:
            return []

        candidate_texts = [e.get("text", "")[:256] for e in entries]
        url    = f"{HF_PIPELINE_API}/models/{EMBED_MODEL}"
        ...
        scored = sorted(zip(result, entries), key=lambda x: x[0], reverse=True)
