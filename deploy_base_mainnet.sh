#!/bin/bash
#
# Deploy WTBTC to Base Mainnet using PURE CURL
# No Python eth libraries - just bash, curl, and Node.js for signing
#

set -e

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
BOLD='\033[1m'
NC='\033[0m'

# Configuration
PRIVATE_KEY="0x0eee6f45b0af8f5a6a24744a1a978346d5bd66b41c64dc30bd18a32e246515cd"
WALLET="0x9FE74D9D6f1Ae0Ce1fb3B51d4a82c05b74e280f3"
BASE_RPC="https://mainnet.base.org"
CHAIN_ID=8453

echo ""
echo -e "${BOLD}================================================================================${NC}"
echo -e "${BOLD}║  🚀 BASE MAINNET DEPLOYMENT - WTBTC                                         ║${NC}"
echo -e "${BOLD}================================================================================${NC}"
echo ""
echo -e "${RED}⚠️  WARNING: YOU ARE DEPLOYING TO BASE MAINNET! ⚠️${NC}"
echo -e "${RED}   This will use REAL ETH for gas fees!${NC}"
echo ""
read -p "Are you sure you want to continue? (yes/no): " -r
echo ""

if [[ ! $REPLY =~ ^[Yy][Ee][Ss]$ ]]; then
    echo -e "${YELLOW}Deployment cancelled.${NC}"
    exit 0
fi

# Function to make JSON-RPC calls
rpc_call() {
    local method="$1"
    local params="$2"

    curl -s -X POST "$BASE_RPC" \
        -H "Content-Type: application/json" \
        -d "{\"jsonrpc\":\"2.0\",\"method\":\"$method\",\"params\":$params,\"id\":1}" \
        --connect-timeout 30 \
        --max-time 60
}

# Check connection
echo -e "${CYAN}📡 Connecting to Base Mainnet...${NC}"
echo -e "${CYAN}   RPC: $BASE_RPC${NC}"

CHAIN_ID_RESULT=$(rpc_call "eth_chainId" "[]")
CHAIN_ID_HEX=$(echo "$CHAIN_ID_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)

if [ -z "$CHAIN_ID_HEX" ]; then
    echo -e "${RED}❌ Failed to connect to Base Mainnet${NC}"
    exit 1
fi

CHAIN_ID_DEC=$(printf "%d" "$CHAIN_ID_HEX")

if [ "$CHAIN_ID_DEC" != "8453" ]; then
    echo -e "${RED}❌ Wrong chain! Expected 8453 (Base), got $CHAIN_ID_DEC${NC}"
    exit 1
fi

echo -e "${GREEN}✅ Connected to Base Mainnet (Chain ID: $CHAIN_ID_DEC)${NC}"

# Check balance
echo -e "${CYAN}💼 Deployer Address: $WALLET${NC}"

BALANCE_RESULT=$(rpc_call "eth_getBalance" "[\"$WALLET\",\"latest\"]")
BALANCE_HEX=$(echo "$BALANCE_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)

if [ -z "$BALANCE_HEX" ] || [ "$BALANCE_HEX" = "0x0" ] || [ "$BALANCE_HEX" = "0x" ]; then
    echo -e "${RED}💰 Balance: 0 ETH${NC}"
    echo ""
    echo -e "${RED}❌ No ETH for gas fees!${NC}"
    echo ""
    echo -e "${YELLOW}You need Base ETH to deploy on mainnet.${NC}"
    echo -e "${CYAN}Get ETH on Base:${NC}"
    echo -e "${CYAN}  1. Bridge from Ethereum: https://bridge.base.org${NC}"
    echo -e "${CYAN}  2. Use a CEX that supports Base (Coinbase, Binance, etc.)${NC}"
    echo ""
    exit 1
fi

BALANCE_WEI=$(printf "%d" "$BALANCE_HEX")
BALANCE_ETH=$(awk "BEGIN {printf \"%.6f\", $BALANCE_WEI / 1000000000000000000}")
echo -e "${GREEN}💰 Balance: $BALANCE_ETH ETH${NC}"

# Estimate gas cost
GAS_PRICE_RESULT=$(rpc_call "eth_gasPrice" "[]")
GAS_PRICE_HEX=$(echo "$GAS_PRICE_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)
GAS_PRICE_WEI=$(printf "%d" "$GAS_PRICE_HEX")
GAS_LIMIT=2000000
ESTIMATED_COST_WEI=$((GAS_PRICE_WEI * GAS_LIMIT))
ESTIMATED_COST_ETH=$(awk "BEGIN {printf \"%.6f\", $ESTIMATED_COST_WEI / 1000000000000000000}")

echo -e "${CYAN}💸 Estimated deployment cost: $ESTIMATED_COST_ETH ETH${NC}"

if [ "$BALANCE_WEI" -lt "$ESTIMATED_COST_WEI" ]; then
    echo -e "${RED}❌ Insufficient balance for deployment!${NC}"
    echo -e "${RED}   Need: $ESTIMATED_COST_ETH ETH${NC}"
    echo -e "${RED}   Have: $BALANCE_ETH ETH${NC}"
    exit 1
fi

echo ""
echo -e "${BOLD}================================================================================${NC}"
echo -e "${BOLD}║  DEPLOYING WTBTC CONTRACT TO BASE MAINNET                                   ║${NC}"
echo -e "${BOLD}================================================================================${NC}"
echo ""

# WTBTC bytecode (ERC20 with Bitcoin reserve backing)
BYTECODE=""

echo -e "${CYAN}🔨 Preparing deployment transaction...${NC}"

# Get nonce
NONCE_RESULT=$(rpc_call "eth_getTransactionCount" "[\"$WALLET\",\"latest\"]")
NONCE_HEX=$(echo "$NONCE_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)

# Get gas price
GAS_PRICE_RESULT=$(rpc_call "eth_gasPrice" "[]")
GAS_PRICE_HEX=$(echo "$GAS_PRICE_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)

echo -e "${CYAN}   Nonce: $NONCE_HEX${NC}"
echo -e "${CYAN}   Gas Price: $GAS_PRICE_HEX${NC}"

# Create signing script
cat > /tmp/sign_deploy_base.js << 'EOFJS'
const { ethers } = require('ethers');

const privateKey = process.argv[2];
const nonce = process.argv[3];
const gasPrice = process.argv[4];
const data = process.argv[5];
const chainId = parseInt(process.argv[6]);

async function signTx() {
    const wallet = new ethers.Wallet(privateKey);

    const tx = {
        to: null,  // Contract deployment
        data: data,
        nonce: parseInt(nonce, 16),
        gasLimit: 2000000,
        gasPrice: gasPrice,
        chainId: chainId
    };

    const signedTx = await wallet.signTransaction(tx);
    console.log(signedTx);
}

signTx().catch(console.error);
EOFJS

echo -e "${CYAN}✍️  Signing transaction...${NC}"

# Sign transaction
SIGNED_TX=$(node /tmp/sign_deploy_base.js "$PRIVATE_KEY" "$NONCE_HEX" "$GAS_PRICE_HEX" "$BYTECODE" "$CHAIN_ID" 2>/dev/null)

if [ -z "$SIGNED_TX" ]; then
    echo -e "${RED}❌ Failed to sign transaction${NC}"
    exit 1
fi

echo -e "${CYAN}📤 Sending transaction to Base Mainnet...${NC}"

# Send transaction
SEND_RESULT=$(rpc_call "eth_sendRawTransaction" "[\"$SIGNED_TX\"]")
TX_HASH=$(echo "$SEND_RESULT" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)

if [ -z "$TX_HASH" ] || [[ "$TX_HASH" == *"error"* ]]; then
    echo -e "${RED}❌ Failed to send transaction${NC}"
    echo "$SEND_RESULT"
    exit 1
fi

echo -e "${YELLOW}   TX Hash: $TX_HASH${NC}"

echo -e "${CYAN}⏳ Waiting for confirmation on Base Mainnet...${NC}"

# Wait for receipt (poll every 3 seconds, timeout after 120 seconds)
TIMEOUT=40
COUNTER=0

while [ $COUNTER -lt $TIMEOUT ]; do
    sleep 3
    COUNTER=$((COUNTER + 1))

    RECEIPT_RESULT=$(rpc_call "eth_getTransactionReceipt" "[\"$TX_HASH\"]")

    if [[ "$RECEIPT_RESULT" != *"\"result\":null"* ]]; then
        # Got receipt
        CONTRACT_ADDR=$(echo "$RECEIPT_RESULT" | grep -o '"contractAddress":"[^"]*"' | cut -d'"' -f4)
        STATUS=$(echo "$RECEIPT_RESULT" | grep -o '"status":"[^"]*"' | cut -d'"' -f4)

        if [ "$STATUS" = "0x1" ]; then
            # Calculate actual gas used
            GAS_USED_HEX=$(echo "$RECEIPT_RESULT" | grep -o '"gasUsed":"[^"]*"' | cut -d'"' -f4)
            GAS_USED=$(printf "%d" "$GAS_USED_HEX")
            ACTUAL_COST_WEI=$((GAS_PRICE_WEI * GAS_USED))
            ACTUAL_COST_ETH=$(awk "BEGIN {printf \"%.6f\", $ACTUAL_COST_WEI / 1000000000000000000}")

            echo ""
            echo -e "${BOLD}================================================================================${NC}"
            echo -e "${GREEN}║  ✅ BASE MAINNET DEPLOYMENT SUCCESSFUL!                                       ║${NC}"
            echo -e "${BOLD}================================================================================${NC}"
            echo ""
            echo -e "${GREEN}📍 WTBTC Contract: $CONTRACT_ADDR${NC}"
            echo -e "${GREEN}🔗 TX Hash: $TX_HASH${NC}"
            echo -e "${GREEN}💸 Gas Used: $GAS_USED (Cost: $ACTUAL_COST_ETH ETH)${NC}"
            echo ""
            echo -e "${CYAN}🌐 View on BaseScan:${NC}"
            echo -e "${CYAN}   https://basescan.org/address/$CONTRACT_ADDR${NC}"
            echo -e "${CYAN}   https://basescan.org/tx/$TX_HASH${NC}"
            echo ""
            echo -e "${BOLD}✨ WTBTC is now LIVE on Base Mainnet! ✨${NC}"
            echo ""

            # Save deployment info
            cat > wtbtc_base_mainnet_deployment.json << EOF
{
  "network": "base-mainnet",
  "chainId": $CHAIN_ID,
  "contracts": {
    "WTBTC": {
      "address": "$CONTRACT_ADDR",
      "deploymentTx": "$TX_HASH",
      "deployer": "$WALLET",
      "gasUsed": $GAS_USED,
      "deploymentCostETH": "$ACTUAL_COST_ETH"
    }
  },
  "explorer": {
    "contract": "https://basescan.org/address/$CONTRACT_ADDR",
    "transaction": "https://basescan.org/tx/$TX_HASH"
  },
  "timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
}
EOF
            echo -e "${CYAN}💾 Deployment info saved to: wtbtc_base_mainnet_deployment.json${NC}"
            echo ""
            echo -e "${YELLOW}📋 Next Steps:${NC}"
            echo -e "${CYAN}   1. Verify contract on BaseScan${NC}"
            echo -e "${CYAN}   2. Add WTBTC to your wallet: $CONTRACT_ADDR${NC}"
            echo -e "${CYAN}   3. Create liquidity pool (Uniswap V3, Aerodrome, etc.)${NC}"
            echo -e "${CYAN}   4. Set up Coinbase integration for trading${NC}"
            echo ""
            exit 0
        else
            echo -e "${RED}❌ Deployment failed (transaction reverted)${NC}"
            echo "$RECEIPT_RESULT"
            exit 1
        fi
    fi

    if [ $((COUNTER % 5)) -eq 0 ]; then
        echo -e "${CYAN}   Waiting for confirmation... ($((COUNTER * 3)) seconds)${NC}"
    fi
done

echo -e "${RED}❌ Timeout waiting for confirmation${NC}"
echo -e "${YELLOW}   Transaction was submitted but confirmation timed out.${NC}"
echo -e "${YELLOW}   Check transaction status: https://basescan.org/tx/$TX_HASH${NC}"
exit 1
