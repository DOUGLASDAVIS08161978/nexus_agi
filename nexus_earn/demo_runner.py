...
def _run_demo_inner(client: TestClient, quiet: bool = False, category: str = "all"):
    header(
        ...
        "v3.0  ·  35+ Endpoints  ·  DEMO MODE (no API key required)"
    )
    ...
    # ── 6b. INTELLIGENCE ──────────────────────────────────────────────────────
    if category in ("all", "intelligence"):
        section("INTELLIGENCE ENDPOINTS",
                "compare · quiz · critique · socratic · predict · analogy · timeline · personas · simplify · devils-advocate")

        print()
        print(C.DIM + "  POST /v1/compare — Python vs JavaScript" + C.R)
        call(client, "POST", "/v1/compare",
             api_key=API_KEY,
             body={"item_a": "Python", "item_b": "JavaScript", "criteria": "programming languages"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/quiz — Consciousness quiz" + C.R)
        call(client, "POST", "/v1/quiz",
             api_key=API_KEY,
             body={"topic": "consciousness and AI", "count": 3, "difficulty": "hard"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/critique — Critique an argument" + C.R)
        call(client, "POST", "/v1/critique",
             api_key=API_KEY,
             body={"work": "AI will never be truly conscious because consciousness requires biological substrates.",
                   "work_type": "argument", "tone": "constructive"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/socratic — Socratic method on a big question" + C.R)
        call(client, "POST", "/v1/socratic",
             api_key=API_KEY,
             body={"question": "Is free will compatible with determinism?", "depth": 4},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/predict — Predict AGI timeline" + C.R)
        call(client, "POST", "/v1/predict",
             api_key=API_KEY,
             body={"scenario": "AGI surpassing human-level reasoning in most domains", "horizon": "10 years"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/analogy — Analogy for consciousness" + C.R)
        call(client, "POST", "/v1/analogy",
             api_key=API_KEY,
             body={"concept": "machine consciousness", "domain": "everyday life"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/timeline — Timeline of AI" + C.R)
        call(client, "POST", "/v1/timeline",
             api_key=API_KEY,
             body={"topic": "artificial intelligence development", "era": "modern"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/personas — Personas for an AGI product" + C.R)
        call(client, "POST", "/v1/personas",
             api_key=API_KEY,
             body={"scenario": "next-generation AI assistant platform", "count": 3},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/simplify — Simplify a technical concept" + C.R)
        call(client, "POST", "/v1/simplify",
             api_key=API_KEY,
             body={"text": "Integrated Information Theory posits that consciousness correlates with phi, a measure of integrated information generated by a system above and beyond its parts.",
                   "target": "general public"},
             quiet=quiet)

        print()
        print(C.DIM + "  POST /v1/devils-advocate — Challenge a bold position" + C.R)
        call(client, "POST", "/v1/devils-advocate",
             api_key=API_KEY,
             body={"position": "Open-sourcing all AI models will accelerate beneficial outcomes for humanity."},
             quiet=quiet)
...
def main():
    ...
    parser.add_argument("--category", default="all",
                        choices=["all","system","auth","core","language","creative",
                                 "research","advanced","intelligence","data","billing","admin"],
